...
# Lazy-loaded globals to support Agent Engine deployment
# (env vars are not available at import time in Agent Engine)
_dataset_id = None
_data_project = None
_compute_project = None
_llm_client = None


def _get_dataset_id():
    """Lazily get BQ_DATASET_ID."""
    global _dataset_id
    if _dataset_id is None:
        _dataset_id = get_env_var("BQ_DATASET_ID")
    return _dataset_id


def _get_data_project():
    """Lazily get BQ_DATA_PROJECT_ID."""
    global _data_project
    if _data_project is None:
        _data_project = get_env_var("BQ_DATA_PROJECT_ID")
    return _data_project


def _get_compute_project():
    """Lazily get BQ_COMPUTE_PROJECT_ID."""
    global _compute_project
    if _compute_project is None:
        _compute_project = get_env_var("BQ_COMPUTE_PROJECT_ID")
    return _compute_project
...
def get_bigquery_schema_and_samples():
    ...
    client = get_bigquery_client(
        project=_get_compute_project(),
        credentials=None,
        user_agent=USER_AGENT,
    )
    dataset_ref = bigquery.DatasetReference(_get_data_project(), _get_dataset_id())
    tables_context = {}
