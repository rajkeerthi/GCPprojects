...
async def call_db_agent(
    ...
    from data_science.sub_agents.bigquery.agent import get_bigquery_agent
...
def _get_bq_execute_sql():
    """Lazily initialize BigQueryToolset for BQML."""
    global _bq_execute_sql
    if _bq_execute_sql is None:
        ...
        _bq_execute_sql = BigQueryToolset(
            tool_filter=bigquery_tool_filter, bigquery_tool_config=bigquery_tool_config
        )
    return _bq_execute_sql


def get_bqml_agent():
    """Lazily initialize and return the BQML agent."""
    global _root_agent
    if _root_agent is None:
        _root_agent = Agent(
            model=os.getenv("BQML_AGENT_MODEL", "gemini-2.5-flash"),  # Default for Agent Engine
            name="bq_ml_agent",
            instruction=return_instructions_bqml(),
            before_agent_callback=setup_before_agent_call,
            tools=[_get_bq_execute_sql(), check_bq_models, call_db_agent, rag_response],
        )
    return _root_agent


# For backwards compatibility
root_agent = None  # Use get_bqml_agent() instead
